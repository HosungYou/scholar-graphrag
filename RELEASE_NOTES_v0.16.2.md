...
> **Commits**: aacd765, 2f217d4

## Summary

Comprehensive 6-layer defense against 401 authentication errors that continued appearing in production logs despite v0.16.1 fixes. The previous per-query guards addressed symptoms (React Query retries) while missing root causes (unguarded polling loops, stale tokens, no global defense, dead sessions staying cached).
...
| 6 | Dead session stays cached after failed refresh | `refetchInterval` creates new queries every 30s, `user` stays truthy | Layer 6 |
...
### Layer 6: Auto-Signout on Dead Session (Critical)
- **File**: `frontend/lib/api.ts`
- When `refreshSession()` fails (no new session), call `supabase.auth.signOut()`
- This sets `user` to `null` via `onAuthStateChange` listener
- All queries with `enabled: !!user` become disabled, stopping `refetchInterval` loops
- Applied to both `request()` and `authenticatedFetch()` methods
- **Impact**: Eliminates the 30s polling loop from `InterruptedImportsSection` and any other `refetchInterval` queries
...
| `frontend/lib/api.ts` | Token refresh + `authenticatedFetch()` + auto-signout on dead session | +47, 6 methods refactored |
...
6. **Dead session test**: Log in → manually expire both tokens → any API call should auto-signout and redirect to login (no 30s polling loop)
